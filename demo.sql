...
SELECT OBTAINED_GRABCOIN, FINAL_PRICE
FROM COMPLETED_TRIP CT
    JOIN TRIP T ON CT.TRIP_ID = T.TRIP_ID;
...
-- Test procedure 3
-- ---------------------------------------------------------
-- TEST CASE: Passenger Trip History
-- ---------------------------------------------------------
SELECT '--- TEST 3: Get Trip History for Passenger 1 (First page, 5 items) ---' AS Scenario;

CALL GET_PASSENGER_TRIP_HISTORY (1, 5, 0);

SELECT '--- TEST 3: Get Trip History for Passenger 5 (who has cancelled trips) ---' AS Scenario;

CALL GET_PASSENGER_TRIP_HISTORY (5, 5, 0);
